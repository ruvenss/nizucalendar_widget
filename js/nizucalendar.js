    ...
    nizu_GetData({ a: "confirmbooking", publickey: nizupublickey, guests: guestsArr, id: nizuapiid, free_slot_id: id, service_id: nizuserviceid, email: window.localStorage.getItem("nizucalc_email"), fullname: window.localStorage.getItem("nizucalc_firstname") + " " + window.localStorage.getItem("nizucalc_familyname"), phone: window.localStorage.getItem("nizutelephone"), phonecode: window.localStorage.getItem("nizucountryCode"), cp: window.localStorage.getItem("nizucp"), city: $("#nizucity").val(), street: window.localStorage.getItem("nizustreet"), place: window.localStorage.getItem("nizuplace"), notes: $("#nizucalnotes").val() }, "Loading...", function (data) {
        if (data.ans>0) {
            ...
            if (data.phoneok===true) {
                ...
                //$(".nizupincontainer").css("display","block");
                //$("#nizuphonepin").focus();
                $("[id^=nizucalstep]").addClass("d-none");
                $("[id^=nizucalstep]").removeClass("d-block");
                $("#nizucalstep5").removeClass("d-none");
            }
        }
    });
}
...
function nizucal_initrender(){
    ...
    if (!document.getElementById(cssId))
    {
        ...
        link.rel  = 'stylesheet';
        link.type = 'text/css';
        link.href = 'https://rgwit.ams3.digitaloceanspaces.com/css/all.min.css';
        link.media = 'all';
        ...
        nizu_GetData({a:"getfreedates",publickey:nizupublickey,id:nizuapiid,service_id:$(this).val()},"Loading...",function(data) {
            if (data.ans>0) {
                ...
                calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    events: nizuholidays.concat(nizufreeslots),
                    validRange: {
                        start: nizucal_today
                    },
                    dateClick: function(info) {
                        ...
                        if ($.inArray(info.dateStr, nizublockslots) > -1) {
                            ...
                            if ($.inArray(info.dateStr, nizufreeblocks) > -1) {
                                nizuselectTime(info.dateStr);
    ...
    calendarEl = document.getElementById('nizucalendar');
    calendar = new FullCalendar.Calendar(calendarEl, {
        locale: 'en' ,
        height: 490,
        selectable: true,
    ...
    $(".nizustepper").on("click", function(){
        if (nizuserviceid>0) {
            switch ($(this).data("step")){
                case 3:
                    if (nizu_confirmid===true){
                        nizucal_rendertab($(this));
                    }
                break;
                default:
                    nizucal_rendertab($(this));
    ...
    $(".nizubtnselecthr").on("click", function(){
        ...
        nizucal_selectedmin=$(this).data("min");
        $(".nizustepper").removeClass("active");
        $("[id^=nizucalstep]").css("display","none");
    ...
    $("#btn_confirm_step5").on("click", function () {
        if (pin.length == 4) {
            nizu_GetData({ a: "confirmpin", publickey: nizupublickey, id: nizuapiid, pin: pin, booking_id: nizu_booking_id }, "Loading...", function (data) {
                if (data.ans == 0) {
                    $(".pin-input").val("");
                } else {
                    $("#nizucalstep5").removeClass("d-block");
                    $("#nizucalstep5").addClass("d-none");
                    $("#nizucalstep6").addClass("d-block");
                    $("#nizucalstep6").removeClass("d-none");
                }
            });
        }
    })
    ...
    $("#btn_back_step_2, .step1").on("click", function () {
        ...
        $("[id^=nizucalstep]").addClass("d-none");
        $("[id^=nizucalstep]").removeClass("d-block");
        $("#nizucalstep1").removeClass("d-none");
        $(".step2").off("click");
    });
    ...
    $("#btn_confirm_calendar").on("click", function () {
        var day_selected = $(".day-selected");
        if (day_selected.length == 1) {
            var day = day_selected.parent().attr("data-value");
            $("[id^=nizucalstep]").addClass("d-none");
            $("[id^=nizucalstep]").removeClass("d-block");
                ...
                if (nizu_ValidateEmail($("#nizucalc_email").val())){
                    if ($("#nizucountryCode").val().length>0) {
                        if ($(".nizutelephone").val().length>0) {
                            ...
                            $(".step2, #btn_back_step_3").on("click", function () {
                                $("#nizustepper").removeClass("neumorphic-slider__line_3");
                                $("#nizustepper").addClass("neumorphic-slider__line_2");
                                $("#nizucalstep3").addClass("d-none");
                                $("#nizucalstep3").removeClass("d-block");
                                $("#nizucalstep2").removeClass("d-none");
    ...
    $("#btn_back_step_4").on("click", function () {
        $("#nizucalstep4").addClass("d-none");
        $("#nizucalstep3").addClass("d-block");
        $("#nizucalstep3").removeClass("d-none");

    });
    $("#btn_confirm_step4").on("click", function () {
        var selected_time = $(".time-selected");
        if (selected_time.length == 1) {
            nizucal_hourselect(selected_time.attr("data-id"), selected_time.attr("data-starttime"));
        }
    });
}
function populatethedays(year, month) {
    ...
    for (var i = 1; i < last_day_of_month; i++) {
        ...
        $(".datecontainer:not(.no-hover)").on("click", function () {
            ...
            $("#btn_confirm_calendar").prop("disabled",false);
        });
    }
...
function nizubuttonselectTime(id) {
    $(".time-selected").removeClass("time-selected");
    var el = $(".nizubtnselecthr[data-id='" + id + "']");
    el.addClass("time-selected");
}
function nizuselectTime(OnthisDate) {
    var selectedDate = nizuformatDate(OnthisDate);
    var nizuselecteddate = new Date(OnthisDate);
    var nizuselectedday=String(nizuselecteddate.getDate()).padStart(2, '0');
    $("#nizutodayweekselected").text(nizuselecteddate.toLocaleString(window.navigator.language, {weekday: 'short'}));
    $("#nizutodayselected").text(nizuselectedday);
    $("#nizutodaymonthselected").text(nizuselecteddate.toLocaleString(window.navigator.language, {month: 'short'}));
    //$("#nizucalendar").css("display","none");
    $("#nizufreeslots").empty();
    nizu_GetData({a:"getfreehours",publickey:nizupublickey,id:nizuapiid,date:selectedDate,service_id:nizuserviceid},"Loading...",function(data) {
        if (data.ans>0) {
            //$("#nizuselecthours").css("display","block");
            for (var i = data.freehours.length - 1; i >= 0; i--) {
                $("#nizufreeslots").append('<button data-starttime="' + data.freehours[i].start + '" data-id="' + data.freehours[i].id + '" class="m-3 nizubtnselecthr btn time-btn col-3" onclick="nizubuttonselectTime(' + data.freehours[i].id + ')"><span class="button_text">' + data.freehours[i].start + ' - ' + data.freehours[i].end + '</span></button>');
        ...
        $(".nizucalframe").load(nizu_serverurl+lang+"/"+nizumobileurl,function(){
            ...
            nizu_GetData({a:"getparameters",publickey:nizupublickey,id:nizuapiid},"Loading...",function(data){
                if (data.ans>0) {
                    ...
                    if (data.holidays){
                        nizuholidays=data.holidays;
                    }
                    if (data.blockslots) {
                        nizublockslots=data.blockslots;
                    }
                    if (data.freeslots) {
                        nizufreeslots=data.freeslots;
...
(c) 2019 Adam Shaw
*/
!function(e,t){"object"==typeof exports&&"undefined"!=typeof module?t(exports,require("@fullcalendar/core")):"function"==typeof define&&define.amd?define(["exports","@fullcalendar/core"],t):t((e=e||self).FullCalendarBootstrap={},e.FullCalendar)}(this,function(e,t){"use strict";var o=function(e,t){return(o=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(e,t){e.__proto__=t}||function(e,t){for(var o in t)t.hasOwnProperty(o)&&(e[o]=t[o])})(e,t)};var r=function(e){function t(){return null!==e&&e.apply(this,arguments)||this}return function(e,t){function r(){this.constructor=e}o(e,t),e.prototype=null===t?Object.create(t):(r.prototype=t.prototype,new r)}(t,e),t}(t.Theme);r.prototype.classes={widget:"fc-bootstrap",tableGrid:"table-bordered",tableList:"table",tableListHeading:"table-active",buttonGroup:"btn-group",button:"btn btn-danger",buttonActive:"active",today:"alert alert-info",popover:"card card-primary",popoverHeader:"card-header",popoverContent:"card-body",headerRow:"table-bordered",dayRow:"table-bordered",listView:"card card-primary"},r.prototype.baseIconClass="fa",r.prototype.iconClasses={close:"fa-times",prev:"fa-chevron-left",next:"fa-chevron-right",prevYear:"fa-angle-double-left",nextYear:"fa-angle-double-right"},r.prototype.iconOverrideOption="bootstrapFontAwesome",r.prototype.iconOverrideCustomButtonOption="bootstrapFontAwesome",r.prototype.iconOverridePrefix="fa-";var a=t.createPlugin({themeClasses:{bootstrap:r}});e.BootstrapTheme=r,e.default=a,Object.defineProperty(e,"__esModule",{value:!0})});